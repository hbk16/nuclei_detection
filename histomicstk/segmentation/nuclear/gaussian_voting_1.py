...
def gaussian_voting_1(I, rmax=35, rmin=10, sSigma=5, Tau=5, bw=15, Psi=0.3):
    ...
    Votes = np.zeros((I.shape[0] + np.int16(2 * r), I.shape[1] + np.int16(2 * r)))
    for i in range(Voting[0].size):
        ...
        try:
            Votes[np.int16(r+muy), np.int16(r+mux)] = Votes[np.int16(r+muy), np.int16(r+mux)] + \
                dMag[Voting[0][i]][Voting[1][i]]
        except Exception:
            pass
    ...
    Votes = Votes[r+np.floor(x.size/2).astype('int16'):-(r+np.ceil(x.size/2).astype('int16')),
                  r+np.floor(x.size/2).astype('int16'):-(r+np.ceil(x.size/2).astype('int16'))]
