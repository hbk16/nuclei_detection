...
def min_model_1(I, Delta=0.3, MaxLength=255, Compaction=3,
    ...
    X, Y, Min, Max = seed_contours_1(I, Delta)
...
def seed_contours(I, Delta=0.3):
    ...
    for i in np.arange(I.shape[0]):
        ...
        if len(Maxima) != len(Minima):
            l = np.min([len(Maxima), len(Minima)])
            Maxima = Maxima[0:l]
            Minima = Minima[0:l]
...
def seed_contours_1(I, Delta=0.3):
    # initialize outputs
    X = []
    Y = []
    Min = []
    ...
    for i in np.arange(I.shape[0]):
        ...
        # calculate gradient
        Gradient = np.abs(I[i, 1:] - I[i, 0:-1])

        # identify local maxima and minimia of row 'i' of 'I'
        Maxima = (((I[i, 1:-1] >= I[i, 0:-2]) &
                  (I[i, 1:-1] > I[i, 2:])).nonzero()[0] + 1).astype('int16')
        Minima = (((I[i, 1:-1] < I[i, 0:-2]) &
                  (I[i, 1:-1] <= I[i, 2:])).nonzero()[0] + 1).astype('int16')

        if len(Maxima) < len(Minima):
            Maxima = np.append(Maxima, I.shape[1] - 1)
        elif len(Maxima) > len(Minima):
            Minima = np.append(Minima, I.shape[1] - 1)

        for j, k in zip(Maxima, Minima):
            p1 = np.min([j, k])
            p2 = np.max([j, k])
            if p2 > p1 + 2 and np.abs(I[i, p1].astype('int16') - I[i, p2].astype('int16')) > Range:
                pos_max_gra = np.median(np.nonzero(Gradient[p1 + 1:p2] == np.max(Gradient[p1 + 1:p2]))[0]).astype('int16') + p1 + 1
                X_.append(pos_max_gra)
                Y_.append(i)
                Max_.append(np.max([I[i, p1], I[i, p2]]))
                Min_.append(np.min([I[i, p1], I[i, p2]]))

        if Maxima[0] < Minima[0]:
            zip1 = Maxima[1:]; zip2 = Minima[:-1]
        else:
            zip1 = Minima[1:]; zip2 = Maxima[:-1]
...
def trace_contours(I, X, Y, Min, Max, MaxLength=255):
    ...
    for i in np.arange(X.size):
        ...
        W = I[max(0, np.int64(Y[i])-np.int64(np.ceil(MaxLength/2.0))):
              min(I.shape[0]+1, np.int64(Y[i])+np.int64(np.ceil(MaxLength/2.0))+1),
              max(0, np.int64(X[i])-np.int64(np.ceil(MaxLength/2.0))):
              min(I.shape[1]+1, np.int64(X[i])+np.int64(np.ceil(MaxLength/2.0)))+1]
        ...
        pX = np.int64(np.ceil(MaxLength/2.0))
        pY = np.int64(np.ceil(MaxLength/2.0))
        ...
        try:
            if cX[0][0] == cX[0][-1] and cY[0][0] == cY[0][-1] and len(cX[0]) <= MaxLength:
                ...
                cXs.append(np.array(cX[0], dtype=np.uint32).copy())
                cYs.append(np.array(cY[0], dtype=np.uint32).copy())
        except Exception as err:
            print(i, err, len(cX), len(cY))
